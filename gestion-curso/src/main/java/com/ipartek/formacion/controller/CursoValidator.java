...
public class CursoValidator implements Validator {
	 ...
	 * "form.nombreRequerido", "tiene que introducirse un nombre");
	 * ValidationUtils.rejectIfEmptyOrWhitespace(errors, "codCurso",
	 * "form.apellidoRequerido", "tienen que introducirse apellidos");
	 * 
	 * Curso curso = (Curso) obj; if (curso.getCodigo() < Curso.CODIGO_NULO) {
	 ...
	 * nombreTamMin, nombreTamMax }, "El nombre tiene la longitud incorrecta");
	 * }
	 * 
	 * if (curso.getApellidos().length() > cursoCodTamMax ||
	 * curso.getApellidos().length() < cursoCodTamMin) {
	 * 
	 * errors.rejectValue("apellidos", "form.apellidosValidado", new Object[] {
	 * cursoCodTamMin, cursoCodTamMax }, "Los apellidos son incorrectos"); }
	 */
